...
window.postFullScreen = function(config) {
	var defaults = {
		...
		buttonText: "Full Screen Mode",
		closeText: "Exit full screen mode",
		appendButtonTo: null,
		beforeInit: null,
		afterInit: null,
		scrollableContent: document.body
	};
	...
	btn.innerHTML = defaults.buttonText;
	btn.onclick = function() {
		div.children[0].style.display = "block";
		defaults.scrollableContent.style.overflow = "hidden";
		return false;
	};
	...
	if (defaults.contentSource) {
		...
		defaults.scrollableContent.appendChild(btn);
	}
